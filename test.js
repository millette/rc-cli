...
if (!process.env.TRAVIS) {
  ...
  test.only('cli (inject)', async (t) => {
    const all = await cli(['https://ici.radio-canada.ca/premiere/emissions/midi-info/segments/entrevue/89953/agriculture-changements-climatiques-production-effets'], true)
    const [one] = all
...
test('No valid items match #3', (t) => t.throwsAsync(findMeta('https://ici.radio-canada.ca/premiere/premiereplus'), 'No items found.'))
